...
from sqlalchemy import (Boolean, Column, ForeignKey, Integer, String, Table,
                        UniqueConstraint, Text)  # Text pode ser melhor para 'groups' JSON
...
class Student(Base):
    ...
    pront: Mapped[str] = mapped_column(String(20), unique=True, nullable=False, index=True)
    ...
    def __repr__(self) -> str:
        ...
        return f"<Student(id={self.id}, pront='{self.pront}', nome='{self.nome[:30]}...')>"  # Trunca nome longo
...
class Reserve(Base):
    ...
    snacks: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False, index=True)
    ...
    student: Mapped["Student"] = relationship(back_populates="reserves", lazy="joined")  # 'joined' pode ser útil aqui
    # Relacionamento Um-para-Um (opcional) com Consumption
    # Uma reserva pode ou não ter um consumo associado
    consumption: Mapped[Optional["Consumption"]] = relationship(back_populates="reserve", uselist=False, lazy="select")
    ...
    __table_args__ = (
        UniqueConstraint("student_id", "data", "snacks", name="_student_date_mealtype_uc",
                         sqlite_on_conflict="IGNORE"),  # IGNORA inserções duplicadas no SQLite
    )

    def __repr__(self) -> str:
        """ Retorna uma representação textual do objeto Reserve. """
        status = "Cancelada" if self.canceled else ("Lanche" if self.snacks else "Almoço")
        return (f"<Reserve(id={self.id}, student={self.student_id}, data='{self.data}', "
...
class Session(Base):
    ...
    __table_args__ = (
        ...
        # UniqueConstraint('refeicao', 'periodo', 'data', 'hora', name="_session_datetime_meal_period_uc"),
    )

    def __repr__(self) -> str:
        """ Retorna uma representação textual do objeto Session. """
        groups_repr = (self.groups[:30] + '...') if self.groups and len(self.groups) > 30 else (self.groups or '[]')
        return (f"<Session(id={self.id}, refeicao='{self.refeicao}', "
...
class Consumption(Base):
    ...
    consumed_without_reservation: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    ...
    reserve_id: Mapped[Optional[int]] = mapped_column(ForeignKey(
        "reserve.id", ondelete="RESTRICT"), nullable=True, index=True)

    # Relacionamento Muitos-para-Um com Student
    student: Mapped["Student"] = relationship(back_populates="consumptions", lazy="joined")  # 'joined' pode ser útil
    # Relacionamento Muitos-para-Um com Session
    session: Mapped["Session"] = relationship(back_populates="consumptions", lazy="joined")  # 'joined' pode ser útil
    # Relacionamento Muitos-para-Um (opcional) com Reserve
    reserve: Mapped[Optional["Reserve"]] = relationship(back_populates="consumption", lazy="select")
    ...
    __table_args__ = (
        UniqueConstraint("student_id", "session_id", name="_student_session_consumption_uc",
                         sqlite_on_conflict="IGNORE"),  # IGNORA duplicatas no SQLite
    )

    def __repr__(self) -> str:
        """ Retorna uma representação textual do objeto Consumption. """
        reserve_info = f"reserve_id={self.reserve_id}" if self.reserve_id else f"sem_reserva={self.consumed_without_reservation}"
        return (f"<Consumption(id={self.id}, student={self.student_id}, session={self.session_id}, "
