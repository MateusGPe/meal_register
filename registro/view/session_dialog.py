...
# Importações locais
from registro.control.constants import (INTEGRATED_CLASSES, SNACKS_JSON_PATH,
                                        NewSessionData, UI_TEXTS)
...
def create_class_checkbox_section(master: tk.Widget, available_classes: List[str]
    ...
    group_frame = ttk.Labelframe(
        master,
        text=UI_TEXTS.get("participating_classes_label", "🎟️ Selecione Turmas Participantes"),
        padding=6
    )
    ...
    if not available_classes:
        ...
        ttk.Label(group_frame, text=UI_TEXTS.get("no_classes_found_db", "Nenhuma turma encontrada no banco de dados.")).grid(
    ...
    group_frame.rowconfigure(tuple(range(num_rows or 1)), weight=1)  # Linhas expansíveis
    ...
    for i, class_name in enumerate(available_classes):
        ...
        check_btn = ttk.Checkbutton(
            ...
            text=class_name,
            variable=check_var,
            bootstyle="success-round-toggle"  # Estilo visual
        )
        check_btn.grid(
...
class SessionDialog(tk.Toplevel):
    ...
    def __init__(self, title: str, callback: Callable[[Union[NewSessionData, int, None]], bool], parent_app: 'RegistrationApp'):
        ...
        self._classes_checkbox_data, self._class_selection_frame = create_class_checkbox_section(
    ...
    def _fetch_available_classes(self) -> List[str]:
        ...
        try:
            ...
            messagebox.showerror(
                ...
                UI_TEXTS.get("error_fetching_classes", "Não foi possível buscar as turmas."),
                parent=self
            )
    ...
    def _on_closing(self):
        ...
        try:
            ...
            logger.exception(f"Erro no callback de fechamento do diálogo: {e}")
    ...
    def _create_section_new_session(self) -> ttk.Labelframe:
        ...
        ttk.Label(master=frame, text=UI_TEXTS.get("meal_type_label", "🍽️ Refeição:")).grid(
        ...
        self._meal_combobox = ttk.Combobox(
            master=frame, values=meal_options, state="readonly", bootstyle='info')
        ...
        ttk.Label(master=frame, text=UI_TEXTS.get("specific_snack_label", "🥪 Lanche Específico:")).grid(
        ...
        self._snack_combobox.config(state='disabled' if self._meal_combobox.get() ==
                                    UI_TEXTS.get("meal_lunch", "Almoço") else 'normal')
    ...
    def _load_snack_options(self) -> Tuple[Set[str], List[str]]:
        ...
        try:
            ...
            if not isinstance(snack_options, list) or not all(isinstance(s, str) for s in snack_options):
                logger.error(f"Conteúdo inválido em '{snacks_path}'. Esperada lista de strings.")
            ...
            logger.warning(
                f"Arquivo de opções de lanche '{snacks_path}' não encontrado. Usando padrão e criando arquivo.")
            # Cria o arquivo com a opção padrão se não existir
            save_json(str(snacks_path), default_options)
            return set(default_options), default_options
        except Exception as e:
            logger.exception(f"Erro ao carregar opções de lanche de '{snacks_path}'.")
    ...
    def _create_section_edit_session(self) -> ttk.Labelframe:
        ...
        frame = ttk.Labelframe(
            self,
            text=UI_TEXTS.get("edit_session_group_label", "📝 Selecionar Sessão Existente para Editar"),
            padding=10
        )
    ...
    def _load_existing_sessions(self) -> Tuple[Dict[str, int], List[str]]:
        ...
        try:
            ...
            sessions: List[SessionModel] = self._session_manager.session_crud.read_all_ordered_by(
            ...
            for s in sessions:
                ...
                try:
                    ...
                    display_date = s.data  # Usa o formato original se a conversão falhar
                # Monta a string de exibição
                display_text = (f"{display_date} {s.hora} - "
                                f"{capitalize(s.refeicao)} (ID: {s.id})")
                sessions_map[display_text] = s.id

            return sessions_map, list(sessions_map.keys())  # Retorna mapa e lista de chaves (display texts)
        except Exception as e:
    ...
    def _create_section_main_buttons(self) -> ttk.Frame:
        ...
        ).grid(row=0, column=1, padx=5, pady=5)  # Coluna 1
        ...
        ttk.Button(
            ...
            bootstyle="danger"
        ).grid(row=0, column=2, padx=5, pady=5)  # Coluna 2
        ...
        ttk.Button(
            ...
            bootstyle="success"
        ).grid(row=0, column=3, padx=5, pady=5)  # Coluna 3
    ...
    def _validate_new_session_input(self) -> bool:
        ...
        try:
            ...
            messagebox.showwarning(
                ...
                UI_TEXTS.get("invalid_time_format", "Formato de hora inválido. Use HH:MM."),
                parent=self)
        ...
        try:
            ...
            messagebox.showwarning(
                UI_TEXTS.get("invalid_input_title", "Entrada Inválida"),
                UI_TEXTS.get("invalid_date_format", "Formato de data inválido. Use {date_format}.").format(
        ...
        if self._meal_combobox.get() not in valid_meals:
            messagebox.showwarning(
                UI_TEXTS.get("invalid_input_title", "Entrada Inválida"),
                UI_TEXTS.get("select_meal_type", "Selecione um Tipo de Refeição válido."),
                parent=self)
            return False
        ...
        if meal_type == UI_TEXTS.get("meal_snack", "Lanche") and not snack_selection:
            messagebox.showwarning(
                UI_TEXTS.get("invalid_input_title", "Entrada Inválida"),
                UI_TEXTS.get("specify_snack_name", "Especifique o nome do lanche para 'Lanche'."),
                parent=self)
        ...
        if not any(var.get() for _, var, _ in self._classes_checkbox_data):
            messagebox.showwarning(
                ...
                UI_TEXTS.get("select_one_class", "Selecione pelo menos uma turma participante."),
                parent=self)
            return False
    ...
    def _save_new_snack_option(self, snack_selection: str):
        ...
        if snack_selection and snack_selection not in self._snack_options_set and "Error" not in snack_selection:
            ...
            logger.info(f"Nova opção de lanche digitada: '{normalized_snack}'. Adicionando à lista.")
            self._snack_options_set.add(normalized_snack)  # Adiciona ao conjunto em memória
            snacks_path = Path(SNACKS_JSON_PATH)
            try:
                # Tenta salvar a lista atualizada (ordenada) no JSON
                if save_json(str(snacks_path), sorted(list(self._snack_options_set))):
                    logger.info(f"Opções de lanche atualizadas e salvas em '{snacks_path}'.")
                    ...
                    messagebox.showerror(
                        ...
                        UI_TEXTS.get("new_snack_save_error", "Não foi possível salvar a nova opção de lanche."),
                        parent=self)
            except Exception as e:
                logger.exception(f"Erro ao salvar nova opção de lanche '{normalized_snack}' em '{snacks_path}'.")
                messagebox.showerror(
                    UI_TEXTS.get("save_error_title", "Erro ao Salvar"),
                    UI_TEXTS.get("unexpected_snack_save_error", "Erro inesperado ao salvar lista de lanches."),
                    parent=self)
    ...
    def _on_okay(self):
        ...
        if session_id_to_load is not None:
            ...
            logger.info(f"Botão OK: Requisitando carregamento da sessão existente ID {session_id_to_load}")
            ...
            try:
                ...
                logger.error(f"Erro ao converter data da UI ({date_ui}) para formato backend: {e}")
            ...
            new_session_data: NewSessionData = {
                "refeição": meal_type,  # Já está no formato esperado (Lanche/Almoço)
    ...
    def _on_sync_reserves(self):
        ...
        self._parent_app.show_progress_bar(True, UI_TEXTS.get(
    ...
    def _sync_monitor(self, thread: SyncReserves):
        ...
        if thread.is_alive():
            ...
            if thread.error:
                logger.error(f"Sincronização de reservas falhou: {thread.error}")
                messagebox.showerror(
                    UI_TEXTS.get("sync_error_title", "Erro de Sincronização"),
                    UI_TEXTS.get("sync_reserves_error_message",
                                 "Falha ao sincronizar reservas:\n{error}").format(error=thread.error),
                    parent=self)
            elif thread.success:
                logger.info("Sincronização de reservas concluída com sucesso.")
                messagebox.showinfo(
                    UI_TEXTS.get("sync_complete_title", "Sincronização Concluída"),
                    UI_TEXTS.get("sync_reserves_complete_message",
                                 "Reservas sincronizadas com sucesso com o banco de dados."),
                    parent=self)
                # Atualiza o combobox de sessões existentes, pois a sincronização pode ter afetado dados
                ...
                messagebox.showwarning(
                    UI_TEXTS.get("sync_status_unknown_title", "Status da Sincronização Desconhecido"),
                    UI_TEXTS.get("sync_reserves_unknown_message", "Sincronização finalizada, mas status incerto."),
                    parent=self)
