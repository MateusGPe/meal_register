...
import json # Para salvar lanches
...
def classes_section(master: tk.Widget, classes: List[str]
    ...
    """ Cria uma seção com checkbuttons para selecionar turmas. """
    ...
    for i, turma_nome in enumerate(classes or []):
        if turma_nome == 'SEM RESERVA': continue # Ignora 'SEM RESERVA' aqui
...
class SessionDialog(tk.Toplevel):
    """ Janela de diálogo para criar ou selecionar uma sessão de serviço de refeição. """
    ...
    def __init__(self, title: str, callback: callable, parent_app: 'RegistrationApp'):
        ...
        available_classes = sorted(
            set(g.nome for g in self.__session_manager.turma_crud.read_all() if g.nome and g.nome != 'Vazio')
        )
    ...
    def on_closing(self):
        """ Lida com o fechamento da janela (clicar no 'X'). """
        logger.info("Diálogo de sessão fechado pelo usuário.")
        self.grab_release()
        self.destroy()
        try: self._callback(None) # Indica cancelamento
        except Exception as e: logger.exception(f"Erro callback fechamento: {e}")

    # --- Widget Creation Sections ---
    def _create_section_new_session(self) -> ttk.Labelframe:
        """ Cria a seção para definir detalhes de uma nova sessão. """
        session_group = ttk.Labelframe(self, text="➕ Nova Sessão", padding=10)
        session_group.columnconfigure(1, weight=1) # Hora/Refeição/Lanche
        session_group.columnconfigure(3, weight=1) # Data
        ...
        self._meal_combobox = ttk.Combobox(master=session_group, values=["Lanche", "Almoço"], state="readonly", bootstyle='info')
        ...
        if snack_options and "Erro" not in snack_options[0]: self._snack_combobox.current(0)
    ...
    def _load_snack_options(self) -> Tuple[Set[str], List[str]]:
        """ Carrega opções de lanche do arquivo JSON. """
        try:
    ...
    def _create_section_class_buttons(self) -> ttk.Frame:
         """ Cria botões de ação para seleção de turmas. """
    ...
    def _create_section_edit_session(self) -> ttk.Labelframe:
        """ Cria a seção para selecionar uma sessão existente. """
        ...
        self._sessions_combobox = ttk.Combobox(master=edit_group, values=session_display_list, state="readonly", bootstyle='dark')
        placeholder = "Selecione para editar..."
        if session_display_list and "Erro" not in session_display_list[0]: self._sessions_combobox.set(placeholder)
        elif session_display_list: self._sessions_combobox.current(0)
    ...
    def _load_existing_sessions(self) -> Tuple[Dict[str, int], List[str]]:
        """ Carrega sessões existentes do DB para o combobox. """
        try:
    ...
    def _create_section_main_buttons(self) -> ttk.Frame:
        """ Cria os botões principais: OK, Cancelar, Sincronizar. """
        button_frame = ttk.Frame(self)
        button_frame.columnconfigure(0, weight=1); button_frame.columnconfigure(4, weight=1) # Espaçadores
        ttk.Button(master=button_frame, text="📥 Sincronizar Reservas", command=self._on_sync_reserves, bootstyle="outline-warning").grid(row=0, column=1, padx=5, pady=5)
        ttk.Button(master=button_frame, text="❌ Cancelar", command=self.on_closing, bootstyle="danger").grid(row=0, column=2, padx=5, pady=5)
        ttk.Button(master=button_frame, text="✔️ OK", command=self._on_okay, bootstyle="success").grid(row=0, column=3, padx=5, pady=5)
        return button_frame
    ...
    def _on_select_meal(self, *_):
        ...
        if is_lunch: self._snack_combobox.set('')
    ...
    def _set_class_checkboxes(self, condition_func: callable):
        """ Define o estado dos checkboxes de turma baseado em uma função condicional. """
        if not hasattr(self, '_classes_chk_data'): return
        for name, var, _ in self._classes_chk_data:
            var.set(condition_func(name, var))

    def _validate_new_session_input(self) -> bool:
        """ Valida os campos de entrada para uma nova sessão. """
        try: datetime.strptime(self._time_entry.get(), '%H:%M')
        except ValueError: messagebox.showwarning("Formato Inválido", "Hora inválida (HH:MM).", parent=self); return False
        try: datetime.strptime(self._date_entry.entry.get(), '%Y-%m-%d')
        except ValueError: messagebox.showwarning("Formato Inválido", "Data inválida.", parent=self); return False
        if not any(var.get() for _, var, _ in self._classes_chk_data):
             messagebox.showwarning("Seleção Inválida", "Selecione pelo menos uma turma.", parent=self); return False
        return True

    def _save_new_snack_option(self, snack_selection: str):
        """ Salva uma nova opção de lanche se necessário. """
    ...
    def _on_okay(self):
        """ Lida com o clique no botão OK (Criar ou Carregar Sessão). """
        selected_session_display = self._sessions_combobox.get()
        session_id_to_load = self.sessions_map.get(selected_session_display) if selected_session_display and "Selecione" not in selected_session_display and "Erro" not in selected_session_display else None

        if session_id_to_load is not None:
            # Carrega sessão existente
            logger.info(f"OK: Carregando sessão ID {session_id_to_load}")
            if self._callback(session_id_to_load): self.grab_release(); self.destroy()
            # else: Mantém diálogo aberto se callback falhar
        else:
            # Cria nova sessão
            logger.info("OK: Criando nova sessão.")
            if not self._validate_new_session_input(): return
            ...
            if meal_type == "Lanche": self._save_new_snack_option(snack_selection)
            ...
            if self._callback(new_session_data): self.grab_release(); self.destroy()
            # else: Mantém diálogo aberto se callback falhar

    def _on_sync_reserves(self):
        """ Inicia a sincronização das reservas (usa barra de progresso da app principal). """
        logger.info("Iniciando sincronização de reservas...")
        # Mostra progresso na janela principal
        self.__parent_app.show_progress_bar(True, "Sincronizando reservas...")
        self.update() # Garante que a UI atualize
    ...
    def _sync_monitor(self, thread: SyncReserves):
        """ Monitora a thread SyncReserves e atualiza a UI principal. """
        if thread.is_alive():
            self.after(100, lambda: self._sync_monitor(thread))
        else:
            # Esconde progresso na janela principal
            ...
            if thread.error:
                ...
                self._update_existing_sessions_combobox() # Atualiza lista de sessões

    def _update_existing_sessions_combobox(self):
        """ Atualiza as opções no combobox de sessões existentes. """
        ...
        else: self._sessions_combobox.set("")